            ...
            <el-col :span="12">
              <el-form ref="form" :model="form" label-width="80px">
                ...
                <el-form-item label-width="100px" class="itemTitle" label="房屋总价" v-if="isFwzj">
                  <el-input style="width: 280px" placeholder="请输入0~99999.99之间的数字" v-model="fwzj"></el-input>
                  <span>万元</span>
                </el-form-item>
                ...
                <el-form-item label-width="100px" class="itemTitle" label="商贷利率" v-if="isLpr">
                  <el-input
                    type="text"
                    style="width:60px;font-color:black"
                    disabled="true"
                    v-model="lpr"
                  ></el-input>
                  <span>%</span>
              ...
              <el-card class="box-card">
                <div slot="header" class="clearfix">
                  <el-row>
                    ...
                    <el-col
                      :span="8"
                      style="text-align: right;margin-left: -40px; margin-right: 20px"
                    >等额本息还款</el-col>
...
export default {
  ...
  methods: {
    open() {
      this.$alert(
        "自2019年10月8日起，新发放的商业住房贷款的贷款利率由“贷款基准利率（4.9%）”转换为“贷款款市场报价利率（LPR）”在LPR的基础上增加基点来确定最终的商贷利率； LPR每月20日（遇节假日顺延）报价一次，可在中国人民银行网站查询。",
        "LPR（贷款市场报价利率）",
        {
    ...
    init() {
      ...
      for (let index = 30; index > 0; index--) {
        let temp = { value: index, label: index };
        templist.push(temp);
      }
      this.years = templist;
    },
    computeMoney(jinge, rate, year) {
      ...
      var N = year * 12;
      var yuegong =
        ((P * R * Math.pow(1 + R, N)) / (Math.pow(1 + R, N) - 1)) * 10000;
    ...
    beforCal(id) {
      if (id == 0) {
        if (this.sdje <= 0) {
          this.$message.warning("贷款金额必填");
          return 0;
        }
        if (this.rate <= 0) {
          this.$message.warning("贷款利率必填");
          return 0;
        }
        if (this.sdyear <= 0) {
          this.$message.warning("贷款年限必填");
          return 0;
        }
      }
      if (id == 1) {
      ...
      if (id == 2) {
        if (this.sdje <= 0 || this.gjjje <= 0) {
          this.$message.warning("贷款金额必填");
          return 0;
        }
        if (this.rate <= 0 || this.gjjll <= 0) {
          this.$message.warning("贷款利率必填");
          return 0;
        }
        if (this.sdyear <= 0 || this.gjjyear <= 0) {
          this.$message.warning("贷款年限必填");
          return 0;
        }
      }
    ...
    onSubmit() {
      ...
      if (this.isZhdk == true) {
        ...
        this.lixiheBen2 =
          parseFloat(this.lixiBen2) + parseFloat(M3) + parseFloat(M2);
