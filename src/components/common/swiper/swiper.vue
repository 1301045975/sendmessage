<template>
    <section class="wh_content" @touchmove="fn">
        ...
        <div v-if="showIndicator" class="wh_indicator">
            <div v-for="(tag,$index) in slidesLength" v-bind:class="{ wh_show_bgcolor: index-1==$index }" :key="'slidesLength' + $index" class="wh_indicator_item"></div>
        </div>
    ...
    export default {
        ...
        mounted() {
            ...
            setTimeout(() => {
                //克隆dom
                this.starDom()
                this.dom.transform = `translate3d(${this.width * -1}px, 0px, 0px)`
                this.dom['-webkit-transform'] = `translate3d(${this.width * -1}px, 0px, 0px)`
                this.dom['-ms-transform'] = `translate3d(${this.width * -1}px, 0px, 0px)`
                if (this.autoPlay) {
                    this.setTime()
                }
            }, 50)
        ...
        methods: {
            ...
            e(x) {
                if (this.slideing && this.t.s != -1) {
                    ...
                    x += this.t.m > 0 ? this.width * 0.3 : this.width * -0.3
                    this.index = Math.round(x / this.width) * -1
                    this.wh('touch')
                }
            },
            setTransform(num) {
            ...
            starDom() {
                ...
                if (this.slidesLength > 1) {
                    ...
                    this.width = document.querySelector('.' + this.className).getBoundingClientRect().width
